...
start_campaign()
{
    launch_campaign()
    {
		...
		campaign="$FUZZER/$TARGET/$ARCID"
		log_name="${LOGDIR}/${FUZZER}_${TARGET}_${ARCID}_container.log"
    ...
    while : ; do
        export CAMPAIGN_CACHEDIR="$CACHEDIR/$FUZZER/$target_bug"
        export CACHECID=$(mutex $MUX_CID \
                get_next_cid "$CAMPAIGN_CACHEDIR")
        export CAMPAIGN_ARDIR="$ARDIR/$FUZZER/$target_bug"
