...
if (_IN_BUFFER_SIZE < sizeof(gchar*) + sizeof(gchar*) + 1) {
	return 0;
}
...
gchar* origin;
origin = malloc(__fuzz_0_LEN + 1);
memcpy(origin, __fuzz_0, __fuzz_0_LEN);
origin[__fuzz_0_LEN] = 0;
gchar* __fuzz_1 = (gchar*) (_IN_BUFFER + _BUF_POINTER);
size_t __fuzz_1_LEN = _IN_REMAIN_LEN;
_BUF_POINTER += __fuzz_1_LEN;

gchar* renamed_to;
renamed_to = malloc(__fuzz_1_LEN + 1);
memcpy(renamed_to, __fuzz_1, __fuzz_1_LEN);
renamed_to[__fuzz_1_LEN] = 0;
...
// Extracted from: gnome-software: plugins/flatpak/gs-flatpak.c_0
// TODO: very hacky. Who knows whether this will work
int __query = 5;
int __context = 5;
int __component = 5;
 __query =xb_silo_lookup_query ( silo,"components[@origin=?]/component/bundle[@type='flatpak'][text()=?]/.." );
xb_value_bindings_bind_str (xb_query_context_get_bindings ( & __context ), 0, origin, NULL );
xb_value_bindings_bind_str (xb_query_context_get_bindings ( & __context ), 1, renamed_to, NULL );
 __component =xb_silo_query_first_with_context(silo,  __query,  & __context, &error);
