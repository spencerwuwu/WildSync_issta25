...
if (_IN_BUFFER_SIZE < sizeof(GBytes*) + 1) {
	return 0;
}
size_t _IN_REMAIN_LEN = (_IN_BUFFER_SIZE - _BUF_POINTER) / 2;

GBytes* __fuzz_0 = (GBytes*) (_IN_BUFFER + _BUF_POINTER);
...
// Extracted from: fwupd: plugins/modem-manager/fu-firehose-updater.c_0
 if ( ( !xb_builder_source_load_bytes(source,  rawprogram, XB_BUILDER_SOURCE_FLAG_WATCH_FILE |
