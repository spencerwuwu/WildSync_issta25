...
if (_IN_BUFFER_SIZE < sizeof(char*) + sizeof(char*) + sizeof(char*) + sizeof(char*) + 1) {
	return 0;
}
...
char* prefix;
prefix = malloc(__fuzz_0_LEN + 1);
memcpy(prefix, __fuzz_0, __fuzz_0_LEN);
prefix[__fuzz_0_LEN] = 0;
char* __fuzz_1 = (char*) (_IN_BUFFER + _BUF_POINTER);
size_t __fuzz_1_LEN = _IN_REMAIN_LEN;
_BUF_POINTER += __fuzz_1_LEN;

char* value;
value = malloc(__fuzz_1_LEN + 1);
memcpy(value, __fuzz_1, __fuzz_1_LEN);
value[__fuzz_1_LEN] = 0;
char* __fuzz_2 = (char*) (_IN_BUFFER + _BUF_POINTER);
size_t __fuzz_2_LEN = _IN_REMAIN_LEN;
_BUF_POINTER += __fuzz_2_LEN;

char* module_name;
module_name = malloc(__fuzz_2_LEN + 1);
memcpy(module_name, __fuzz_2, __fuzz_2_LEN);
module_name[__fuzz_2_LEN] = 0;
char* __fuzz_3 = (char*) (_IN_BUFFER + _BUF_POINTER);
size_t __fuzz_3_LEN = _IN_REMAIN_LEN;
_BUF_POINTER += __fuzz_3_LEN;

char* name;
name = malloc(__fuzz_3_LEN + 1);
memcpy(name, __fuzz_3, __fuzz_3_LEN);
name[__fuzz_3_LEN] = 0;
...
// Extracted from: sysrepo: src/ly_wrap.c_0
 if ( (lyd_new_opaq(tree, ctx,  name,  value,  prefix,  module_name, &tree) ) ){
}
