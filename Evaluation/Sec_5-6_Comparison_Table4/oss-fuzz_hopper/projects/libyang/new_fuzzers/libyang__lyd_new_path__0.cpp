...
char* __vty__rpc_paramsi_xpath;
__vty__rpc_paramsi_xpath = malloc(__fuzz_0_LEN + 1);
memcpy(__vty__rpc_paramsi_xpath, __fuzz_0, __fuzz_0_LEN);
__vty__rpc_paramsi_xpath[__fuzz_0_LEN] = 0;
char* __fuzz_1 = (char*) (_IN_BUFFER + _BUF_POINTER);
size_t __fuzz_1_LEN = _IN_REMAIN_LEN;
_BUF_POINTER += __fuzz_1_LEN;

char* __vty__rpc_paramsi_value;
__vty__rpc_paramsi_value = malloc(__fuzz_1_LEN + 1);
memcpy(__vty__rpc_paramsi_value, __fuzz_1, __fuzz_1_LEN);
__vty__rpc_paramsi_value[__fuzz_1_LEN] = 0;
...
// Extracted from: frr: lib/northbound_cli.c_0
// TODO: very hacky. Who knows whether this will work
int __vty__num_rpc_params = 5;
 for ( size_t i = 0;
 i < __vty__num_rpc_params;
 i ++ ){
 err =lyd_new_path(tree, ctx,  __vty__rpc_paramsi_xpath,  __vty__rpc_paramsi_value,  0, &tree);
}
