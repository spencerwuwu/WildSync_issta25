...
if (_IN_BUFFER_SIZE < sizeof(struct ly_in*) + 1) {
	return 0;
}
size_t _IN_REMAIN_LEN = (_IN_BUFFER_SIZE - _BUF_POINTER) / 2;

struct ly_in* __fuzz_0 = (struct ly_in*) (_IN_BUFFER + _BUF_POINTER);
...
// Extracted from: libnetconf2: src/session_server.c_0
 struct ly_in * msg;
 r = __fuzz_0;
 if ( r == -1 ){
}
 if ( ( !lyd_parse_op(ctx, tree,  msg, LYD_JSON,  LYD_TYPE_RPC_NETCONF, &tree, &tree) ) ){
}
