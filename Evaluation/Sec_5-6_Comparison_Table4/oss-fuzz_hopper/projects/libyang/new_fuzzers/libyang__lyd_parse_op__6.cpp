...
char* notif_json;
notif_json = malloc(__fuzz_0_LEN + 1);
memcpy(notif_json, __fuzz_0, __fuzz_0_LEN);
notif_json[__fuzz_0_LEN] = 0;
...
// Extracted from: sysrepo: src/plugins/ntf_json.c_0
 struct ly_in * in = NULL;
ly_in_new_memory ( notif_json, & in );
 if ( (lyd_parse_op(ctx, tree,  in, LYD_JSON,  LYD_TYPE_NOTIF_YANG, &tree, &tree) ) ){
}
