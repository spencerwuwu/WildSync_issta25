...
// Extracted from: libnetconf2: src/session_client.c_0
// TODO: very hacky. Who knows whether this will work
int __session__ctx = 5;
 if (ly_ctx_get_module_implemented ( __session__ctx,"ietf-yang-library" ) ){
}
 if ( ( mod =ly_ctx_get_module_implemented ( __session__ctx,"ietf-netconf" ) ) && !lys_feature_value ( mod,"xpath" ) ){
}
 if (ly_ctx_get_module_implemented ( __session__ctx,"ietf-netconf-nmda" ) ){
}
 if ( xpath_support ){
}
 if ( (lyd_validate_all(&tree, ctx,  LYD_VALIDATE_PRESENT, &tree) ) ){
}
