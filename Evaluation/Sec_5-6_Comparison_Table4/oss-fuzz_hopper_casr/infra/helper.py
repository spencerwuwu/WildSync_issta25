...
def get_parser():  # pylint: disable=too-many-statements,too-many-locals
  ...
  run_fuzzer_parser.add_argument(
      '--storage-dir', help='directory to store corpus for the fuzz target')
  run_fuzzer_parser.add_argument('--my_store',
                                 default=False,
                                 action='store_true')
  run_fuzzer_parser.add_argument('--fuzztime', help='time in seconds to fuzz')
  run_fuzzer_parser.add_argument('--container-name', help='name of the given container')
  run_fuzzer_parser.add_argument('--cpu', help='docker container CPU affinity')
  ...
  #                             'report')
  #coverage_parser.add_argument('--my-corpus',
  #                             help='specify location of my corpus'
  #                             ' to be used ')
  coverage_parser.add_argument('--hopper_out', help="compile directory for hopper binary")
  ...
  #                             'corpus using wget')
  coverage_parser.add_argument('--container-name', help='name of the given container')
  coverage_parser.add_argument('--cpu', help='docker container CPU affinity')
  ...
  introspector_parser.add_argument('--seconds',
                                   ...
                                   default=10)
  introspector_parser.add_argument('--container-name', help='name of the given container')
  ...
  introspector_parser.add_argument(
      '--combine-public',
      help='if specified, will use both public corpora and new harnesses with --my-corpora for code coverage',
      default=False,
      action='store_true')
  introspector_parser.add_argument('--my-corpus', help="base_path to my_corpus")
  introspector_parser.add_argument('--skip-fuzz-targets', 
                                   default="",
                                   help="skip fuzzing targets, seperated by ','")
  introspector_parser.add_argument('--hopper_out', help="compile directory for hopper binary")
  introspector_parser.add_argument(
      '--new-bin',
      default=False,
      action='store_true')
  introspector_parser.add_argument('--cpu', help='docker container CPU affinity')
...
def build_image_impl(project, cache=True, pull=False, architecture='x86_64'):
  ...
  image_name = 'hopper-casr_%s' % (image_name)
...
def _get_my_fuzz_targets(project, skips=[]):
  ...
  for name in os.listdir(project.out):
    ...
    if name == 'llvm-symbolizer':
      continue
    if not name.startswith(project.name + "__"):
      continue
...
def _get_fuzz_targets(project, only_existing=False, skips=[]):
  """Returns names of fuzz targest build in the project's /out directory."""
  fuzz_targets = []
  for name in os.listdir(project.out):
    if name in skips:
    ...
    if name == 'llvm-symbolizer':
      continue
    if only_existing:
      if name.startswith(project.name + "__"):
        continue
...
def download_corpora(args, only_existing=False, skips=[]):
  ...
  if args.fuzz_target:
    ...
    fuzz_targets = _get_fuzz_targets(args.project, only_existing=only_existing, skips=skips)
  ...
  for fuzz_target in fuzz_targets:
    ret = _download_for_single_target(fuzz_target)
    if not ret:
      return False
  return True


def coverage(args, new_bin=False):
  ...
  if args.cpu:
    run_args.extend([
      f"--cpuset-cpus={args.cpu}",
      f"--env=AFFINITY={args.cpu}"
    ])

  if args.container_name:
    run_args.extend(['--name', args.container_name])
  ...
  #elif args.my_corpus:
  #  if not os.path.exists(args.my_corpus):
  #    logger.error('The path provided in --my-corpus argument does not '
  #                 'exist.')
  #    return False
  #  corpus_dir = os.path.realpath(args.my_corpus)
  #  run_args.extend(['-v', '%s:/corpus' % (corpus_dir)])
  #else:
  #  run_args.extend(['-v', '%s:/corpus' % args.project.corpus])

  if new_bin:
    run_args.extend(['-v', './new_bin:/new_bin'])
  ...
  if new_bin:
    run_args.append('/new_bin/coverage')
  else:
...
def _introspector_prepare_corpus(args):
  ...
  if args.private_corpora or args.public_corpora:
    ...
    pass
    #skips = args.skip_fuzz_targets.split(",")
    #fuzzer_targets = _get_my_fuzz_targets(args.project, skips=skips)
    #for fuzzer_name in fuzzer_targets:
    #  fuzzer_corpus_dir = f"./my_corpus/{args.project.name}/{fuzzer_name}"
    #  if not os.path.exists(fuzzer_corpus_dir):
    #    logger.error(f'{fuzzer_corpus_dir} does not exist yet')
    #    return False
    #if args.combine_public:
    #  corpora_command = ['download_corpora', '--public']
    #  corpora_command.append(args.project.name)
    #  if not download_corpora(parse_args(parser, corpora_command), only_existing=True, skips=skips):
    #    logger.error('Failed to download corpora')
    #    return False
    #  target_corpus_dir = "build/corpus/%s" % args.project.name
    #  for corpus in os.listdir(target_corpus_dir):
    #    if corpus in fuzzer_targets:
    #      continue
    #    if corpus in skips:
    #      continue
    #    dest_dir = f"./my_corpus/{args.project.name}/{corpus}"
    #    if os.path.exists(dest_dir):
    #      shutil.rmtree(dest_dir)
    #    shutil.copytree(os.path.join(target_corpus_dir, corpus), dest_dir)
  else:
    ...
    for fuzzer_name in fuzzer_targets:
      ...
      run_fuzzer_command = [
          'run_fuzzer', '--sanitizer', 'address', '--storage-dir',
...
def introspector(args):
  ...
  coverage_command = [
      'coverage', 
      "--hopper_out", args.hopper_out
      #'--no-corpus-download', '--port', ''
  ]
  if args.container_name:
    coverage_command.extend(['--container-name', args.container_name])

  # Collect coverage.
  #if not args.my_corpus:
  #  coverage_command.extend([args.project.name])
  #else:
  #  coverage_command.extend(['--my-corpus', args.my_corpus, args.project.name]) 
  coverage_command.extend([args.project.name])
  if args.new_bin:
    new_bin = True
  else:
    new_bin = False
  if not coverage(parse_args(parser, coverage_command), new_bin=new_bin):
...
def run_fuzzer(args):
  ...
  if args.storage_dir:
    if not os.path.exists(args.storage_dir):
      ...
      #logger.error('The path provided in --storage-dir argument does not exist')
      #return False
    storage_dir = os.path.realpath(args.storage_dir)
    if args.engine == 'libfuzzer':
      run_args.extend([
          '-v',
          '{storage_dir}:/tmp/{fuzzer}_corpus'.format(storage_dir=storage_dir,
                                                               fuzzer=args.fuzzer_name)
      ])
    elif args.engine == 'afl':
      run_args.extend([
          '-v',
          '{storage_dir}:/tmp/{fuzzer}_{engine}_out'.format(storage_dir=storage_dir,
                                                               fuzzer=args.fuzzer_name,
                                                               engine=args.engine)
      ])
  ...
  if args.fuzztime:
    if args.engine == 'libfuzzer':
      args.fuzzer_args.extend([
          f'-max_total_time={args.fuzztime}', '-detect_leaks=0'
      ])
    elif args.engine == 'afl':
      run_args.extend([
        f"--env=TIMEOUT={args.fuzztime}"
      ])
  ...
  run_args.extend(['-v', './new_bin:/new_bin'])
  run_args.extend(['--env=RUN_FUZZER_MODE=auto_mode'])
  run_args.extend([
      '-v',
      '%s:/out' % args.project.out,
      '-t',
