...
if (_IN_BUFFER_SIZE < sizeof(struct lys_module*) + sizeof(lyd_diff_cb) + 1) {
	return 0;
}
lyd_diff_cb __fuzz_1;
memcpy(&__fuzz_1, _IN_BUFFER + _BUF_POINTER, sizeof(lyd_diff_cb));
_BUF_POINTER += sizeof(lyd_diff_cb);
...
int __lyd_diff_cb = 5;
 if ( (lyd_diff_apply_module(&tree, tree,  mod,  diff_cb,  NULL) ) ){
}
