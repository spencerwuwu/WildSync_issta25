...
if (_IN_BUFFER_SIZE < sizeof(LYD_ANYDATA_VALUETYPE) + sizeof(union lyd_any_value*) + 1) {
	return 0;
}
LYD_ANYDATA_VALUETYPE __fuzz_0;
memcpy(&__fuzz_0, _IN_BUFFER + _BUF_POINTER, sizeof(LYD_ANYDATA_VALUETYPE));
_BUF_POINTER += sizeof(LYD_ANYDATA_VALUETYPE);

LYD_ANYDATA_VALUETYPE value_type;
value_type = __fuzz_0;
size_t _IN_REMAIN_LEN = (_IN_BUFFER_SIZE - _BUF_POINTER) / 2;

union lyd_any_value* __fuzz_1 = (union lyd_any_value*) (_IN_BUFFER + _BUF_POINTER);
...
// Extracted from: sysrepo: src/ly_wrap.c_0
 if ( (lyd_any_copy_value(tree,  value,  value_type) ) ){
}
