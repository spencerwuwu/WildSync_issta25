...
if (_IN_BUFFER_SIZE < sizeof(enum lyd_type) + 1) {
	return 0;
}
enum lyd_type __fuzz_0;
memcpy(&__fuzz_0, _IN_BUFFER + _BUF_POINTER, sizeof(enum lyd_type));
_BUF_POINTER += sizeof(enum lyd_type);
...
// Extracted from: sysrepo: src/ly_wrap.c_0
 if ( (lyd_validate_op(tree, tree,  op_type, &tree) ) ){
}
