...
if (_IN_BUFFER_SIZE < sizeof(struct lys_module*) + sizeof(char*) + sizeof(char*) + 1) {
	return 0;
}
size_t _IN_REMAIN_LEN = (_IN_BUFFER_SIZE - _BUF_POINTER) / 4;

struct lys_module* __fuzz_0 = (struct lys_module*) (_IN_BUFFER + _BUF_POINTER);
...
// Extracted from: sysrepo: src/ly_wrap.c_1
 if ( (lyd_new_term(tree,  mod,  name,  value,  0, &tree) ) ){
}
