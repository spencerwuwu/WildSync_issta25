...
int LLVMFuzzerTestOneInput(const uint8_t *d, size_t size) {
  ...
  // START of test_dict_deflate
  {
    ...
    if (err != Z_STREAM_END) {
        fprintf(stderr, "deflate dict should report Z_STREAM_END\n");
        //return 0;
        goto end_test_dict_deflate;
    }
    err = deflateEnd(&c_stream);
    CHECK_ERR(err, "deflateEnd");

  }
  // END of test_dict_deflate
  end_test_dict_deflate:

  //test_dict_inflate(compr, comprLen);
  // START of test_dict_inflate
  {
  int err;
  z_stream d_stream; /* decompression stream */
  unsigned char *uncompr;
  ...
  // END of test_dict_inflate
  end_test_dict_inflate:
