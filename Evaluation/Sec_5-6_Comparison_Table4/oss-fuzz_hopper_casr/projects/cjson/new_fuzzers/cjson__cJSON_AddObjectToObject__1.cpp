...
// Extracted from: mosquitto: plugins/dynamic-security/groups.c_1
 if ( (cJSON_AddStringToObject ( json,"command","getGroup" ) == NULL || ( j_data =cJSON_AddObjectToObject(json, "data") ) == NULL || ( correlation_data &&cJSON_AddStringToObject ( json,"correlationData", correlation_data ) == NULL ) ) ){
}
