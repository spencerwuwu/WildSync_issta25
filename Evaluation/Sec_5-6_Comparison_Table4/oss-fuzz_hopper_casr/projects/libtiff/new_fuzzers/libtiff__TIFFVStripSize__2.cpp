...
if (_IN_BUFFER_SIZE < sizeof(uint32_t) + sizeof(uint32_t) + 1) {
	return 0;
}
uint32_t __fuzz_0;
...
// Extracted from: graphicsmagick: tiff/tools/tiffcrop.c_0
 uint32_t row, nrows, rowsperstrip;
 for ( row = 0;
 row < imagelength;
 row += rowsperstrip ){
 nrows = ( __fuzz_0 + rowsperstrip > imagelength ) ? imagelength - row:
 rowsperstrip;
