...
if (_IN_BUFFER_SIZE < sizeof(uint16_t) + 1) {
	return 0;
}
uint16_t __fuzz_0;
memcpy(&__fuzz_0, _IN_BUFFER + _BUF_POINTER, sizeof(uint16_t));
_BUF_POINTER += sizeof(uint16_t);

uint16_t nBitsPerPixel;
nBitsPerPixel = __fuzz_0;
...
// Extracted from: graphicsmagick: tiff/contrib/addtiffo/tif_overview.c_1
TIFFErrorExt (TIFFClientdata(tif), "  TIFFBuildOverviews  ", "  File `%s' has samples of %d bits per sample.  Sample   \n   " "  sizes of less than 8 bits per sample are not supported.   \n   ",TIFFFileName ( tif ), nBitsPerPixel );
