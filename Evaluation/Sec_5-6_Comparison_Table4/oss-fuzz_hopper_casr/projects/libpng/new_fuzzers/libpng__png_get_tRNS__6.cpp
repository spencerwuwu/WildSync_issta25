...
// Extracted from: sdl2_image: external/libpng/contrib/libtests/pngvalid.c_0
// TODO: very hacky. Who knows whether this will work
int __PNG_INFO_tRNS = 5;
 png_bytep trans_alpha;
 int num;
 if (png_get_PLTE ( png_handler.png_ptr, png_handler.info_ptr, & pal, npalette ) & PNG_INFO_PLTE ){
 if ( i <= 0 || i > 256 ){
}
}
 else{
 ...
 trans_alpha = 0;
 num = 2;
 if ( ( (png_get_tRNS(png_handler.png_ptr, png_handler.info_ptr,  & trans_alpha,  & num,  0) & __PNG_INFO_tRNS ) != 0 && ( trans_alpha != NULL || num != 1 ) && ! ( trans_alpha != NULL && num == 0 ) ) ){
}
