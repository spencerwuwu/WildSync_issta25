...
if (_IN_BUFFER_SIZE < sizeof(XbNode*) + 1) {
	return 0;
}
size_t _IN_REMAIN_LEN = (_IN_BUFFER_SIZE - _BUF_POINTER) / 2;

XbNode* __fuzz_0 = (XbNode*) (_IN_BUFFER + _BUF_POINTER);
...
// Extracted from: fwupd: src/fu-engine.c_0
 if (xb_node_get_attr ( n,"date_eol" ) != NULL ){
}
xb_node_query_first_full( n, query, &error);
