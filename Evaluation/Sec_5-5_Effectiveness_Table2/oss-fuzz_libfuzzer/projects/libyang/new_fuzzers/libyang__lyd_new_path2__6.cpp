...
if (_IN_BUFFER_SIZE < sizeof(char*) + sizeof(uint32_t) + sizeof(char*) + 1) {
	return 0;
}
uint32_t __fuzz_1;
...
char* value;
value = malloc(__fuzz_0_LEN + 1);
memcpy(value, __fuzz_0, __fuzz_0_LEN);
value[__fuzz_0_LEN] = 0;
char* __fuzz_2 = (char*) (_IN_BUFFER + _BUF_POINTER);
size_t __fuzz_2_LEN = _IN_REMAIN_LEN;
_BUF_POINTER += __fuzz_2_LEN;

char* path;
path = malloc(__fuzz_2_LEN + 1);
memcpy(path, __fuzz_2, __fuzz_2_LEN);
path[__fuzz_2_LEN] = 0;
...
// Extracted from: sysrepo: src/ly_wrap.c_0
 if ( (lyd_new_path2(tree, ctx,  path,  value,  value ?strlen ( value ):
 0,  LYD_ANYDATA_STRING,  options, &tree, &tree) ) ){
}
