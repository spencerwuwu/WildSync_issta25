...
if (_IN_BUFFER_SIZE < sizeof(uint32_t) + sizeof(char*) + sizeof(char*) + 1) {
	return 0;
}
uint32_t __fuzz_0;
...
char* value;
value = malloc(__fuzz_2_LEN + 1);
memcpy(value, __fuzz_2, __fuzz_2_LEN);
value[__fuzz_2_LEN] = 0;
...
// Extracted from: frr: lib/northbound.c_0
 err =lyd_new_path(tree, ctx,  xpath,  value,  options, &tree);
