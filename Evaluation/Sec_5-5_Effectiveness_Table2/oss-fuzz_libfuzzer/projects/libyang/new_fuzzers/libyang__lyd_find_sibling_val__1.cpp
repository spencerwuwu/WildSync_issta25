...
if (_IN_BUFFER_SIZE < sizeof(struct lysc_node*) + sizeof(char*) + 1) {
	return 0;
}
size_t _IN_REMAIN_LEN = (_IN_BUFFER_SIZE - _BUF_POINTER) / 3;

struct lysc_node* __fuzz_0 = (struct lysc_node*) (_IN_BUFFER + _BUF_POINTER);
...
// Extracted from: sysrepo: src/ly_wrap.c_0
 lyrc =lyd_find_sibling_val(tree,  schema,  value,  value ?strlen ( value ):
 0, &tree);
