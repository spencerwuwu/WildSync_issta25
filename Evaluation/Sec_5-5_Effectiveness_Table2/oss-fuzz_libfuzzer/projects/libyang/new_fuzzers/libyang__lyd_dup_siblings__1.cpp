...
if (_IN_BUFFER_SIZE < sizeof(uint32_t) + sizeof(struct lyd_node*) + sizeof(int) + 1) {
	return 0;
}
uint32_t __fuzz_0;
...
struct lyd_node* __fuzz_1 = (struct lyd_node*) (_IN_BUFFER + _BUF_POINTER);
...
// Extracted from: sysrepo: src/ly_wrap.c_1
 if ( siblings ){
 lyrc =lyd_dup_siblings(tree,  ( struct lyd_node_inner * ) parent,  options, &tree);
}
