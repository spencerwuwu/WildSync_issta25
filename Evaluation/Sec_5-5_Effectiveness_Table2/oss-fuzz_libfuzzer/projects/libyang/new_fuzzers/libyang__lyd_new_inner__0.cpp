...
int __rpc__type = 5;
int __rpc_gen__has_data = 5;
int __op2__schema__nodetype = 5;
int __session__ctx = 5;
 struct lys_module * mod;
 char * module_name = NULL, * rpc_name = NULL, * module_check = NULL;
 switch ( __rpc__type ){
 if ( __rpc_gen__has_data ){
{
 if ( __op2__schema__nodetype & ( LYS_RPC | LYS_ACTION ) ){
}
}
}
 rpc_name ="get-config";
 ...
 mod =ly_ctx_get_module_implemented ( __session__ctx, module_name );
 lyrc =lyd_new_inner(tree,  mod,  rpc_name,  0, &tree);
}
