...
if (_IN_BUFFER_SIZE < sizeof(uint32) + sizeof(uint32) + 1) {
	return 0;
}
uint32 __fuzz_0;
memcpy(&__fuzz_0, _IN_BUFFER + _BUF_POINTER, sizeof(uint32));
_BUF_POINTER += sizeof(uint32);

uint32 __fuzz_1;
memcpy(&__fuzz_1, _IN_BUFFER + _BUF_POINTER, sizeof(uint32));
_BUF_POINTER += sizeof(uint32);
...
// Extracted from: imagemagick: coders/tiff.c_0
// TODO: very hacky. Who knows whether this will work
int __uint32 = 5;
 uint32 tile_columns, tile_rows;
 tile_columns = ( uint32 ) __fuzz_0;
 tile_rows = ( uint32 ) __fuzz_1;
TIFFDefaultTileSize(tif,  & tile_columns,  & tile_rows);
