...
if (_IN_BUFFER_SIZE < sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint32_t) + 1) {
	return 0;
}
uint32_t __fuzz_0;
...
uint32_t __fuzz_1;
memcpy(&__fuzz_1, _IN_BUFFER + _BUF_POINTER, sizeof(uint32_t));
_BUF_POINTER += sizeof(uint32_t);

uint32_t stripinplane;
stripinplane = __fuzz_1;
uint32_t __fuzz_2;
memcpy(&__fuzz_2, _IN_BUFFER + _BUF_POINTER, sizeof(uint32_t));
_BUF_POINTER += sizeof(uint32_t);

uint32_t rowsperstrip;
rowsperstrip = __fuzz_2;
...
// Extracted from: graphicsmagick: tiff/libtiff/tif_read.c_0
 uint32_t rows;
 rows = __fuzz_0 - stripinplane * rowsperstrip;
 if ( rows > rowsperstrip ) rows = rowsperstrip;
