...
if (_IN_BUFFER_SIZE < sizeof(double) + sizeof(double) + sizeof(double) + 1) {
	return 0;
}
double __fuzz_0;
...
double scale_level_h;
scale_level_h = __fuzz_1;
double __fuzz_2;
memcpy(&__fuzz_2, _IN_BUFFER + _BUF_POINTER, sizeof(double));
_BUF_POINTER += sizeof(double);

double scale_level_w;
scale_level_w = __fuzz_2;
...
// Extracted from: pqiv: pqiv.c_0
 double scale_level = __fuzz_0 > scale_level_h ? scale_level_h:
 scale_level_w;
 if ( scale_level > 1. ){
 scale_level = 1.;
}
