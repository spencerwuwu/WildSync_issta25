...
// Extracted from: mosquitto: apps/mosquitto_ctrl/dynsec.c_1
 cJSON * j_acl;
 j_acl =cJSON_CreateObject ( );
 if ( (cJSON_AddStringToObject ( j_acl,"acltype", type ) == NULL ||cJSON_AddStringToObject ( j_acl,"topic", topic ) == NULL ||cJSON_AddBoolToObject( j_acl, "allow", require_termination) == NULL ) ){
}
