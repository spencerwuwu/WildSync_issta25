...
// Extracted from: sysrepo: examples/oper_data_push_example.c_0
 char * path, * filepath = NULL, * value = NULL;
 if ( argc > 2 ){
}
 if (strrchr ( path,' .' ) ){
 filepath = path;
 ...
 if ( filepath ){
 rc =lyd_parse_data_path(ctx,  filepath, LYD_JSON,  LYD_PARSE_ONLY | LYD_PARSE_STRICT, LYD_VALIDATE_PRESENT, &tree);
}
{
}
