...
if (_IN_BUFFER_SIZE < sizeof(char*) + sizeof(uint32_t) + 1) {
	return 0;
}
uint32_t __fuzz_1;
...
char* data_path;
data_path = malloc(__fuzz_0_LEN + 1);
memcpy(data_path, __fuzz_0, __fuzz_0_LEN);
data_path[__fuzz_0_LEN] = 0;
...
// Extracted from: sysrepo: src/ly_wrap.c_0
 if ( data_path ){
 lyrc =lyd_parse_data_path(ctx,  data_path, LYD_JSON,  parse_options, LYD_VALIDATE_PRESENT, &tree);
}
