...
// Extracted from: gnome-software: plugins/core/gs-plugin-appstream.c_0
// TODO: very hacky. Who knows whether this will work
int __old_thread_default = 5;
 gchar * test_xml;
 if ( __old_thread_default ==g_main_context_default ( ) ){
}
 if ( __old_thread_default != NULL ){
}
 if ( __old_thread_default != NULL ){
}
 ...
 test_xml = __fuzz_0;
 if ( test_xml != NULL ){
 if ( ( !xb_builder_source_load_xml(source,  test_xml, XB_BUILDER_SOURCE_FLAG_WATCH_FILE |
				     XB_BUILDER_SOURCE_FLAG_LITERAL_TEXT, &error) ) ){
}
}
{
}
