...
if (_IN_BUFFER_SIZE < sizeof(yyjson_mut_val*) + 1) {
	return 0;
}
size_t _IN_REMAIN_LEN = (_IN_BUFFER_SIZE - _BUF_POINTER) / 2;

yyjson_mut_val* __fuzz_0 = (yyjson_mut_val*) (_IN_BUFFER + _BUF_POINTER);
...
// Extracted from: fastfetch: src/modules/packages/packages.c_0
// TODO: very hacky. Who knows whether this will work
int __options__disabled = 5;
int __defaultOptions_disabled = 5;
 if ( __options__disabled != __defaultOptions_disabled ){
 yyjson_mut_val * arr =yyjson_mut_obj_add_arr(mdoc,  module, "disabled");
}
