...
// Extracted from: rspamd: src/libstat/backends/sqlite3_backend.c_0
// TODO: very hacky. Who knows whether this will work
int __bk__in_transaction = 5;
 int wal_frames, wal_checkpointed, mode;
 if ( __bk__in_transaction ){
}
 mode = SQLITE_CHECKPOINT_TRUNCATE;
 mode = SQLITE_CHECKPOINT_RESTART;
 mode = SQLITE_CHECKPOINT_FULL;
 if ( (sqlite3_wal_checkpoint_v2(cx.db,  NULL,  mode,  & wal_frames,  & wal_checkpointed) != SQLITE_OK ) ){
}
